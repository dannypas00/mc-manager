<template>
  <nav class="flex items-center justify-between border-t border-gray-200 px-4 sm:px-0">
    ...
    <div class="hidden md:-mt-px md:flex">
      ...
      <template v-if="rightPages[rightPages.length - 1] !== lastPage && currentPage !== lastPage">
        <!-- End -->
        <PaginationEntry
          :index="lastPage"
...
const leftPages: ComputedRef<number[]> = computed(() => _.range(
  Math.max(1, currentPage.value - props.paginationLimitLeft),
  currentPage.value,
).filter((page: number) => page !== currentPage.value));

const rightPages: ComputedRef<number[]> = computed(() => _.range(
  currentPage.value + 1,
  props.lastPage + 1,
).filter((page: number) => page !== currentPage.value));
